...
class UnityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.executable_args.update(executable_args)
        self.recording_options.update(recording_options)
        ...
        if use_editor:
            ...
            # ipdb.set_trace()
            self.comm = comm_unity.UnityCommunication(
                port=str(self.port_number), **self.executable_args)

        atexit.register(self.close)

        print("--- Unity Environment created ---")
        self.reset()
    ...
    def reset(self, environment_graph=None, environment_id=None, init_rooms=None):
        ...
        for i in range(self.num_agents):
            if i in self.agent_info:
                self.comm.add_character(
                    self.agent_info[i], initial_room=rooms[i])
            else:
                self.comm.add_character()
        ...
        self.changed_graph = True
        graph = self.get_graph()
        self.rooms = [(node['class_name'], node['id'])
                      for node in graph['nodes'] if node['category'] == 'Rooms']
    ...
    def get_observations(self):
        ...
        for agent_id in range(self.num_agents):
            ...
            dict_observations[agent_id] = self.get_observation(
                agent_id, obs_type)
    ...
    #         dict_action_space[agent_id] = [node['id']
    ...
    def get_observation(self, agent_id, obs_type, info={}):
        if obs_type == 'partial':
            ...
            camera_ids = [self.num_static_cameras + agent_id *
                          self.num_camera_per_agent + self.CAMERA_NUM]
            ...
            s, images = self.comm.camera_image(
                camera_ids, mode=current_mode, image_width=image_width, image_height=image_height)
            if not s:
                pdb.set_trace()
            return images[0]
        else:
